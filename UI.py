...
class main_window(tk.Tk):
    def button1_on_click(self):
        ...
        f0, timeaxis, sp, ap = change.world_decompose(wav)
        MCEP = utils.world_encode_spectral_envelop(sp, 16000)
        ax[1].imshow(MCEP.T)
        ax[1].set_aspect('auto')
        ax[1].set_title('original MCEP')
        ax[1].set_xticks([])
        ax[1].set_yticks([])
        ...
        ax[0].set_xticks([])
        ax[0].set_yticks([])
        ax[1].set_title('transformed MCEP')
        ax[1].set_xticks([])
        ax[1].set_yticks([])
